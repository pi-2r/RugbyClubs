...
$smat = mysql_real_escape_string($_POST['smat']);
...
$clubactuel = mysql_real_escape_string($_POST['nomclub']);
$niveau = mysql_real_escape_string($_POST['niveau']);
$mniveau = mysql_real_escape_string($_POST['mniveau']);
$spe = mysql_real_escape_string($_POST['specialite']);
$taille = mysql_real_escape_string($_POST['taille']);
$poid = mysql_real_escape_string($_POST['poid']);
$poste = mysql_real_escape_string($_POST['poste']);
$postes = $_POST['postes'][0].','.$_POST['postes'][1];
...
$idclub = mysql_query("SELECT id FROM `clubs` WHERE `nom`='$clubactuel'")or die(mysql_error());
...
$presence = mysql_query("SELECT `id` FROM `users_joueur` WHERE `mail_jou`='$mail'")or die(mysql_error());
if (mysql_num_rows($presence) == 0) // Le club n'est pas inscrit, on continue
{
	...
	$requete = "INSERT INTO `users_joueur`(civilite_jou, nom_jou, prenom_jou, mail_jou,adresse_jou, E_id_ville, E_id_pays, tel, password, ddn, statusmat, pseudo, photo, E_id_club, E_id_niveau_actuel, E_id_niveau_meilleur, spe, E_id_poste_pref,E_ids_poste_second, offreco, taille, poid, valide) VALUES('$civilite','$nom','$prenom','$mail','$adresse','$idville','$nation','$tel','$pass','$ddn','$smat','$pseudo','$photo','$idclub','$niveau','$mniveau','$spe','$poste','$postes','$offreco','$taille','$poid', '$valide')";
	mysql_query($requete)or die(mysql_error());
	$idusersclub = mysql_insert_id(); // Id du club inscrit

	// On stoque son adresse IP
	$ip = $_SERVER["REMOTE_ADDR"];
	$requete = "INSERT INTO `ips_joueur`(E_id_club, ip) VALUES('$idusersclub', '$ip')";
	mysql_query($requete)or die(mysql_error());
